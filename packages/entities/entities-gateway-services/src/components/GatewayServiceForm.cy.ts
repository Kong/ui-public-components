  ...
  describe('Konnect', () => {
    ...
    const interceptUpdate = (status = 200): void => {
      cy.intercept(
        {
          method: 'POST',
          url: `${baseConfigKonnect.apiBaseUrl}/api/control_planes/${baseConfigKonnect.controlPlaneId}/v1/schemas/json/service/validate`,
        },
        {
          statusCode: status,
          body: { },
    ...
    it('should update correct Port value based on selected Protocol', () => {
      ...
      cy.getTestId('k-select-input').click()
      cy.getTestId('k-select-item-tls_passthrough').click()
      cy.getTestId('gateway-service-port-input').should('have.value', gatewayService1.port)

      // Protocol - grpc; Port - 80
      cy.getTestId('k-select-input').click()
      cy.getTestId('k-select-item-grpc').click()
      cy.getTestId('gateway-service-port-input').should('have.value', gatewayService1.port)
      ...
      // Protocol - ws; Port - 80
      cy.getTestId('k-select-input').click()
      cy.getTestId('k-select-item-ws').click()
      cy.getTestId('gateway-service-port-input').should('have.value', gatewayService1.port)
      ...
      cy.getTestId('k-select-input').click()
      cy.getTestId('k-select-item-udp').click()
      cy.getTestId('gateway-service-port-input').should('have.value', gatewayService1.port)
    })
