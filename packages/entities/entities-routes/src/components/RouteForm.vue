    ...
    >
      ...
      >
        ...
        <RouteFormConfigTabs
          ...
          :router-flavors="routerFlavors"
        >
          <template
            v-if="routerFlavors.traditional && routerFlavors.expressions && (!recordFlavor || recordFlavor !== currentConfigTab)"
            #before-content
          >
            <KAlert
          ...
          <template
            ...
            #traditional
          >
            <template v-if="!recordFlavor || recordFlavor === RouterFlavor.TRADITIONAL">
              ...
              <!-- Traditional Router Advanced Fields -->
              <KCollapse
          ...
          <template
            ...
            #expressions
          >
            <template v-if="!recordFlavor || recordFlavor === RouterFlavor.EXPRESSIONS">
              <KCard v-if="state.fields.protocols">
                <template #title>
                  <div class="route-form-routing-rules-title-container">
                    <span
              ...
              <!-- Expressions Router Advanced Fields -->
              <KCollapse
                v-model="isAdvancedFieldsCollapsed"
                class="route-form-advanced-fields-collapse"
                trigger-alignment="leading"
...
const props = defineProps({
  ...
  routerFlavors: {
    type: Object as PropType<RouterFlavors>,
    required: false,
    default: () => ({
      traditional: true,
    }),
...
const currentConfigTab = ref<string>(props.routerFlavors.traditional ? RouterFlavor.TRADITIONAL : props.routerFlavors.expressions ? RouterFlavor.EXPRESSIONS : '')
const recordFlavor = ref<RouterFlavor | undefined>(undefined)
const isAdvancedFieldsCollapsed = ref<boolean>(true)

const payloadFlavor = computed<RouterFlavor | undefined>(() => {
  if (currentConfigTab.value) {
    return currentConfigTab.value as RouterFlavor
  }

  if (props.routerFlavors.traditional) {
    return RouterFlavor.TRADITIONAL
  } else if (props.routerFlavors.expressions) {
    return RouterFlavor.EXPRESSIONS
  }
...
const state = reactive<RouteState<BaseRouteStateFields>>({
  routerFlavors: props.routerFlavors,
  fields: {
    name: '',
...
const updateFormValues = (data: Record<string, any>): void => {
  if (props.routeId) {
    const flavor = typeof data.expression === 'string' && data.expression.length > 0 ? RouterFlavor.EXPRESSIONS : RouterFlavor.TRADITIONAL
