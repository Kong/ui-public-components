import type { KonnectRouteFormConfig, KongManagerRouteFormConfig, RouterFlavors } from '../types'
...
const TRADITIONAL_ONLY: RouterFlavors = { traditional: true, expressions: false }
const EXPRESSIONS_ONLY: RouterFlavors = { traditional: false, expressions: true }
const TRADITIONAL_EXPRESSIONS: RouterFlavors = { traditional: true, expressions: true }

const formatRouteFlavors = (routerFlavors?: RouterFlavors): string => {
  return routerFlavors ? [...routerFlavors.traditional ? ['trad'] : [], ...routerFlavors.expressions ? ['expr'] : []].join('+') || 'none' : 'default'
}
...
describe('<RouteForm />', { viewportHeight: 700, viewportWidth: 700 }, () => {
  describe('Kong Manager', () => {
    ...
    // Tests 4 possible RouterFlavors: <not-set>, <trad only>, <expr only>, <trad+expr>
    for (const routerFlavors of [undefined, TRADITIONAL_ONLY, EXPRESSIONS_ONLY, TRADITIONAL_EXPRESSIONS]) {
      const configTabs = `tabs=${formatRouteFlavors(routerFlavors)}`
      ...
      it(`should show create form, ${configTabs}`, () => {
        ...
        cy.getTestId('route-form-config-tabs')
          .should(routerFlavors?.traditional && routerFlavors?.expressions ? 'be.visible' : 'not.exist')
        ...
        if (routerFlavors?.traditional && routerFlavors?.expressions) {
          // trad + expr 2 tabs
          // switch to trad tab
          cy.get('#traditional-tab').click()
        } // else: we will be on the trad tab by default

        if (routerFlavors?.traditional) {
      ...
      if (!routerFlavors || routerFlavors?.traditional) {
  ...
  describe('Konnect', { viewportHeight: 700, viewportWidth: 700 }, () => {
    ...
    // Tests 2 possible RouterFlavors: <not-set>, <trad only>
    for (const routerFlavors of [undefined, TRADITIONAL_ONLY]) {
      const configTabs = `tabs=${formatRouteFlavors(routerFlavors)}`
